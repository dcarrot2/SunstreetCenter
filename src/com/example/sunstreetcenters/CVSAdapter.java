...
import android.widget.TextView;
	 ...
	 */
public class CVSAdapter extends ArrayAdapter<State> {
		
		Context ctx;
		
		//We must accept the textViewResourceId parameter, but it will be unused
		//for the purposes of this example.
		public CVSAdapter(Context context, int textViewResourceId, String ext) {
		 ...
		 */
		private void loadArrayFromFile(String ext){
			try {
				// Get input stream and Buffered Reader for our data file.
				InputStream is = ctx.getAssets().open(ext); 
				BufferedReader reader = new BufferedReader(new InputStreamReader(is));
				//string that represents each line
				String line;
